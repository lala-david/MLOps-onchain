...
for _ in tqdm(range(1), desc="Clustering"):
    clusters = dbscan.fit_predict(scaled_X)

mapped_clusters = [cluster_map.get(c, 'noise') for c in clusters]

plt.figure(figsize=(10, 6))
sns.scatterplot(x=X_tsne[:, 0], y=X_tsne[:, 1], hue=mapped_clusters, palette="coolwarm", legend="full")
plt.title('ETH Onchain DBSCAN')
...
ax.set_title('ETH Onchain DBSCAN (3D)')
plt.show()
